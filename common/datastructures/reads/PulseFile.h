...
class PulseFile : public PulseBaseCommon {
 public:
    ...
    void CopySignal(HalfWord *signalData, // either a vector or matrix
      ...
      if (signalNDims == 1) {
        for (i = 0; i < readLength; i++) {
          readData[i] = signalData[pulseIndex + baseToPulseIndex[i] ];
        }
      }
      else {
        for (i = 0; i < readLength; i++) {
    ...
    template<typename T_FieldType>
      void StoreField(vector<T_FieldType> &source, unsigned int *basToPlsIndex, T_FieldType *dest, int destLength) {
      int i;
    ...
    void CopyReadAt(int holeNumber, 
                    unsigned int *baseToPulseIndex, // index from pulse to base from the beginning of the read.
                    SMRTSequence &read, int readStartPos, int alignedSequenceLength) {

      int pulseIndex = pulseStartPositions[holeNumber];
      bool allocResult;
      if (midSignal.size() > 0) {
