 ...
 public:
  ...
  void AddPlatformInformation(PlatformType _platform) {
    ...
    if (pulseDataGroup.Initialize(outFile.rootGroup, "PulseData") == 0) {
      cout << "Could not create file " << _hdfFileName << ".  Error creating group /PulseData." << endl;
      exit(1);
    }
      
    pulseDataGroup.AddGroup("BaseCalls"); 
    if (baseCallGroup.Initialize(pulseDataGroup, "BaseCalls") == 0) {
      cout << "Could not create file " << _hdfFileName << ".  Error creating group /PulseData/BaseCall." << endl;
      exit(0);
    }
    ...
    if (zmwGroup.Initialize(baseCallGroup, "ZMW") == 0) {
      cout << "Could not create file " << _hdfFileName << ".  Error creating group /PulseData/BaseCall/ZMW." << endl;
      exit(0);
    }
    ...
    if (scanDataGroup.Initialize(outFile.rootGroup, "ScanData") == 0) {
      cout << "Could not create file " << _hdfFileName << ".  Error creating /ScanData." << endl;
      exit(0);
    }

    scanDataGroup.AddGroup("RunInfo");
    if (runInfoGroup.Initialize(scanDataGroup, "RunInfo") == 0) {
      cout << "Could not creat file " << _hdfFileName << ". Error creating /ScanData/RunInfo"<< endl;
      exit(0);
    }
