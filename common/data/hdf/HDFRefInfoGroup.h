...
class HDFRefInfoGroup {
 public:
  ...
  bool Create(HDFGroup &parent) {
    ...
    masterDatasetAtom.Create(refInfoGroup.group, "MasterDataset", "ID");
    ...
    nRow.Create(refInfoGroup.group, "nRow");
    return true;
  }
  ...
  int Initialize(HDFGroup &parentGroup) {
    ...
    if (nRow.Initialize(refInfoGroup, "nRow") == 0) { return 0; }    
    return 1;
  }
  ...
  void Read(RefInfo &refInfo) {
    refInfo.nRow = fullNameArray.size();
    refInfo.fullName.resize(refInfo.nRow);
    refInfo.id.resize(refInfo.nRow);
    refInfo.length.resize(refInfo.nRow);
    refInfo.md5.resize(refInfo.nRow);
    /*
